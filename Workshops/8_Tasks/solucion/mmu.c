...
paddr_t mmu_init_kernel_dir(void) {
  ...
  kpd[0].attrs = MMU_P | MMU_W;       // U/S = 0, P = 1, R/W = 1 
...
void mmu_map_page(uint32_t cr3, vaddr_t virt, paddr_t phy, uint32_t attrs) {
  ...
  if (page_dir[dir_idx].pt == 0 && page_dir[dir_idx].attrs == 0) {
    ...
    page_dir[dir_idx].attrs = MMU_P | MMU_U | MMU_W;
  } 
...
paddr_t mmu_init_task_dir(paddr_t phy_start) {
  ...
  mmu_map_page(dir_phy, (vaddr_t)TASK_STACK_BASE - PAGE_SIZE, pila, MMU_P | MMU_U | MMU_W);
